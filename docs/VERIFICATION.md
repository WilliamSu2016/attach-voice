...
## SRT 文件导入、编辑与导出（F14，待实现）

F14 完成后，以下场景必须纳入验证。它们不改变 A1-A5 的视频和配音验收口径，但用于保证
SRT 元数据不会被 TTS 合成流程破坏：

| 场景 | 通过条件 |
|---|---|
| SRT 导入 | UTF-8/UTF-8 BOM 的多条、多行字幕可导入；起止时间以毫秒精度保留；非法时间、空文本、乱序或重叠字幕给出条目/行号，且当前表格不变 |
| 编辑与导出 | 导入后编辑起始时间、结束时间和文本，导出的独立 `.srt` 保留编辑后的文本与毫秒级起止时间；序号和换行风格可规范化 |
| 与 TTS 解耦 | 合成或重新生成配音后，`subtitle_end_time` 不变，`duration` 为实际音频时长；字幕结束时间不截断或变速音频 |
| 现有旁白规则 | 配音仍从字幕起始时间开始，按下一段起始时间或视频结束时间截断；视频画面时长、编码策略和现有手工分段流程不变 |
| 导入替换安全 | 当前表格已有内容时，替换前必须确认；解析失败或取消时，表格内容和已生成配音均不变 |

## 视频内嵌字幕轨（F15，需求已确认，待实现/验证）

下列场景是未来验收要求，不是已执行证据。新增测试和脚本在实现 F15 时编写；
不得将 F14 的 SRT 文件往返结果当作 MP4 内嵌字幕已完成的证据。

| 场景 | 通过条件 |
|---|---|
| 默认关闭与兼容 | 新建窗口「内嵌字幕」默认关闭；关闭后走原有导出行为，SRT 独立导出及音频试听/导出不变 |
| 仅添加字幕 | 不生成配音也能导出；有原声时保留其内容，无音轨时仍无音轨，不调用 TTS；音轨可复制时保持原音轨 |
| 配音一起导出 | replace/overlay 两种模式均能加入字幕轨，原声音量设置及配音时间轴不变 |
| 真实 MP4 字幕轨 | ffprobe 确认新增一条 `mov_text` 字幕轨及 default disposition；提取字幕复核中文、多行文本与毫秒起止时间，误差不超过 0.001s |
| 当前编辑快照 | 修改文本、起止时间后再次导出，字幕随当前表格变化；已有字幕结束时间不被 TTS 实际时长覆盖；缺失结束时间沿用 F14 规则 |
| 视频与时间边界 | 真实封装前后视频时长误差 < 0.05s；复制模式视频压缩包载荷一致（不能仅靠同编码名称判断）；字幕结束超出视频时不延长视频、不修改表格及独立 SRT |
| 错误与取消 | 无视频、字幕为空或非法时明确报错，不产生声称成功却缺字幕的结果；取消后 UI 恢复，无残留本次导出进程或临时文件 |
| 播放器实际操作 | 在支持 MP4 `mov_text` 的播放器中实际打开输出文件，能选择新增字幕轨并显示/关闭字幕；记录播放器名称、版本和观察结果，不要求所有播放器自动显示一致 |

计划命令见 `feature_list.json` F15。播放器检查为人工项，必须实际操作或由用户确认；
容器探测、字幕提取和 GUI 脚本不能替代播放器的显示/关闭观察。既有 A4 记录仍仅证明原有
音视频播放，不证明新增软字幕在 VLC、WMP 或浏览器中普遍可用。
